...
class Scanner:
    ...
    def skip_white_space(self):
        ...
        regex = re.compile("\s+")
        result = True
        while self.current_char_index < len(self.input_string) and result:
            self.current_char_index += 1
            if self.current_char_index >= len(self.input_string):
                return
            result = regex.match(self.input_string[self.current_char_index])
    ...
    def get_token(self):
        ...
        if token == None:
            monkey = re.compile("\S+")
            if monkey.match(self.input_string[self.current_char_index]):
                self.no_token()
    ...
    def consume(self, *expected_tokens):
        ...
        if this_token[0] in expected_tokens:
            self.current_token = self.get_token()
            if this_token[0] == Token.ID or this_token[0] == Token.NUM:
                return (this_token[0], this_token[1])
            else:
                return this_token[0]
        else:
            self.unexpected_token(this_token, expected_tokens)
...
class Token:
    ...
    token_regexp = [
        ...
        (DIV,   '\\/'), # / is special in regular expressions
