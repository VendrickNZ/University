...
class FrequencyList(object):
    ...
    def add(self, letter, frequency=1):
        ...
        >>> test.add('b', 3)
        >>> test
        Frequency List -> Frequency('a', 3) -> Frequency('b', 6) -> None
        ...
        if self.head == None:
            ...
            while current != None:
                if current.letter == letter:
                    current.frequency += new_node.frequency
                    return   
...
def filter_possible_chars(corpus, last):
    ...
    [' ']
    >>> filter_possible_chars('monkey want banana', 'on')
    ['k']
    """
    # ---start student section---
    character_list = []
    ...
    start = 0
    while index >= 0:
        index = corpus.find(last, start)
        start = index + 1
        try:
            if index >= 0:
                character_list.append(corpus[index+2])
        except:
            pass
...
def select_next_guess(possibles):
    ...
    temp = highest
    if highest != None:
        possibles.remove(highest.letter)
    if temp != None:
        return temp.letter
    else:
        return
