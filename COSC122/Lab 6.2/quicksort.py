...
def quicksort_range_helper(values, left, right, start, end, style):
    ...
    value_range = range(left, right)
    overlap = set(number_range)
    if overlap.intersection(value_range):
        return quicksort_helper(values, left, right, style)
    # ===end student section===
...
def pivot_index_mo3(values, left, right):
    ...
    if right == left + 1:
        ...
        if (values[left] >= values[middle]) != (values[left] >= values[right]):
            return left
        elif (values[middle] >= values[left]) != (values[middle] >= values[right]):
            return middle
        else:
            return right
    # ===end student section===
