...
class KdTree:
    ...
    def points_in_range(self, query_rectangle):
        ...
        """
        # Replace me with a full implementation
        if node is self.is_leaf:
            matches = all points in node.points lying within rectangle
        else:
            matches = []
            if rectangle ∩ left (or bottom) half space is not empty:
                matches += points_in_range(node.left, query_rectangle)
            if rectangle ∩ right (or top) half space is not empty:
