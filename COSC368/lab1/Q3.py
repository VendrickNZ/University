...
clear_button = ttk.Button(frame_right, text="Clear",
                          command=lambda: clear_data(user_input))
...
for _ in range(26):
    if i == 0:
        button = ttk.Button(
            frame_bottom_row1, text=board[i][j], width=3, command=lambda x=board[i][j]: append(user_input, x))
        button.grid(row=i, column=j, padx=1, pady=1)
    if i == 1:
        button = ttk.Button(
            frame_bottom_row2, text=board[i][j], width=3, command=lambda x=board[i][j]: append(user_input, x))
        button.grid(row=i, column=j, padx=1, pady=1)
    if i == 2:
        button = ttk.Button(
            frame_bottom_row3, text=board[i][j], width=3, command=lambda x=board[i][j]: append(user_input, x))
        button.grid(row=i, column=j, padx=1, pady=1)
    j += 1
    if j >= len(board[i]):
        i += 1
